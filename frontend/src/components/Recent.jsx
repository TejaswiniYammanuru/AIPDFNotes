...
const Recent = () => {
  ...
  const processedPdfs = () => {
    ...
    if (sortBy === 'date') {
      filtered.sort((a, b) => {
        const dateA = new Date(a.last_modified || a.updated_at);
        const dateB = new Date(b.last_modified || b.updated_at);
        return dateB - dateA;
      });
    } else if (sortBy === 'name') {
      filtered.sort((a, b) => {
        if (!a.pdfname) return 1;
        if (!b.pdfname) return -1;
        return a.pdfname.localeCompare(b.pdfname);
      });
    } else if (sortBy === 'size') {
      filtered.sort((a, b) => (b.pdf_size || 0) - (a.pdf_size || 0));
    }
    
    return filtered;
  };

  const filteredPdfs = processedPdfs();
        ...
        <div className="mb-6 border-b border-gray-200 pb-5">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-5">
            <div className="mb-4 md:mb-0">
              <h1 className="text-2xl font-semibold text-gray-800">Recent Documents</h1>
              <p className="text-gray-500 mt-1 text-sm">Your recently viewed or modified PDFs</p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center">
              <label htmlFor="daysFilter" className="text-xs text-gray-500 mr-2">Time range:</label>
              <select
                id="daysFilter"
                className="px-3 py-1.5 rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500 text-sm bg-white"
                value={daysFilter}
            ...
            {/* Limit filter */}
            <div className="flex items-center">
              <label htmlFor="limitFilter" className="text-xs text-gray-500 mr-2">Show:</label>
              <select
                id="limitFilter"
                className="px-3 py-1.5 rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500 text-sm bg-white"
                value={limitFilter}
            ...
            {/* Sort filter */}
            <div className="flex items-center">
              <label htmlFor="sortFilter" className="text-xs text-gray-500 mr-2">Sort by:</label>
              <select
                id="sortFilter"
                className="px-3 py-1.5 rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500 text-sm bg-white"
                value={sortBy}
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-shrink-0">
                        <img src={pdfIcon} alt="PDF" className="h-10 w-8 object-contain" />
                      </div>
                      <div>
                        ...
                        <div className="flex items-center text-xs text-gray-500 mt-1">
                          ...
                          <span>{formatTimeSince(pdf.last_modified || pdf.updated_at)}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mt-2 mb-3">
                    ...
                    <div className="flex items-center">
                      ...
                      <span>{formatDate(pdf.created_at || pdf.updated_at)}</span>
                    </div>
                  </div>
